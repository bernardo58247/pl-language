...
echo "=== instalando PL no Termux / installing PL on Termux ==="
...
string t(const string& key) {
    static map<string, map<string, string>> msgs = {
        ...
        {"banner", {
            {"pt", "--- PL (Portuguese Language) Nativa [Termux] ---\ndigite 'sair' para encerrar.\n"},
            {"en", "--- PL (Portuguese Language) Native [Termux] ---\ntype 'sair' or 'exit' to quit.\n"},
            {"es", "--- PL (Portuguese Language) Nativa [Termux] ---\nescriba 'sair' o 'salir' para terminar.\n"}
        }}
    };
...
echo "instalado com sucesso no Termux! / successfully installed on Termux!"
